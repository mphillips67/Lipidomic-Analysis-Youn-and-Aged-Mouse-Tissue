...
#read in data 
sol <- read_excel("Hinton_Muscle-Lipdomics.xlsx", sheet=4)  #read in excel sheet with relevant data
...
same_values_rows <- which(apply(sol_lipids[, 2:8], 1, function(row) all(row == row[1])))
#turns out we have some columns where all samples have the same value. These have to be removed for pca analysis. This line identifies the columns (there are 2 for the sol data).
sol_lipids <- sol_lipids[-same_values_rows, ]
...
sol_lipids$lipids <- fix #al fixes are done using regex. for a given data set, you will have to look at the names in your orignal table and figure out how to adjust them to the naming system lipidr accepts. 

all_match <- lipidr::annotate_lipids(sol_lipids[[1]])
bad_match <- all_match %>% filter(not_matched)
good_match <-subset(all_match, not_matched == "FALSE")

##create sample annotation table
Sample <- colnames(sol_lipids[,2:ncol(sol_lipids)]) #create a vector with all sample names

SampleType <- c(rep("Young",4), rep("Old",4)) #create vector where each entry describes corresponding sample. 

sol_annotation <- cbind(Sample,SampleType)

write.table(sol_annotation, "Soleus_annotation.txt",  quote = FALSE, row.names = FALSE, sep="\t") #print out sol annotation file

###create lipidR objects
sol_lipids_exp <- as_lipidomics_experiment(sol_lipids, normalized = TRUE, logged = TRUE)

#list unmatched lipids  and add sample annotation 
non_parsed_molecules(sol_lipids_exp)
sol_lipids_exp <- add_sample_annotation(sol_lipids_exp,  "Soleus_annotation.txt")
sol_lipids_exp <- remove_non_parsed_molecules(sol_lipids_exp)

#pca
mvaresults <- mva(sol_lipids_exp, measure="Area", method="PCA")
...
de_old_young <- de_analysis(sol_lipids_exp, Young - Old )
sig <- significant_molecules(de_old_young, p.cutoff = 0.05, logFC.cutoff = 1) #list of sig results
write.table(sig, "Soleus_sig_lipids.txt",  quote = FALSE, row.names = FALSE, sep="\t") #print out sol sig lipids

plot_results_volcano(de_old_young, show.labels =FALSE) + xlim(-8,8) + theme(strip.text.x = element_text(size = 20)) + theme(plot.title = element_text(size = 25, face = "bold")) +  theme(legend.text = element_text(size=15)) + theme(legend.title = element_text(size=18)) +theme(axis.text=element_text(size=18), axis.title=element_text(size=20,face="bold")) + ylim(0,2)
...
write.table(enrich_results[,c(1:4,8)], "Sol_lipidset.txt",  quote = FALSE, row.names = FALSE, sep="\t") #print out enrichment results
...
#clustering analysis
##all classes
plot_heatmap(sol_lipids_exp, layout=  list(font = list(size = 8)) ) 
...
enriched_heatmap <-  rowData(sol_lipids_exp)$Class %in% enriched_classes
sol_lipids_exp_enriched <- sol_lipids_exp[enriched_heatmap,]

sol_lipids_exp_enriched@metadata[["dimnames"]] <- c("Lipids","Sample")

plot_heatmap(sol_lipids_exp_enriched, cluster_col="none", cluster_row = "none", layout = list(font = list(size = 20)))
